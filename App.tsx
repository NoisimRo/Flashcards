...
function AppContent() {
  ...
  const handleEditDeck = async (updatedDeck: Deck) => {
    ...
    try {
      ...
      // If deck has new cards (e.g., generated by AI), add them to backend
      const oldDeck = decks.find(d => d.id === updatedDeck.id);
      if (oldDeck && updatedDeck.cards.length > oldDeck.cards.length) {
        // Find newly added cards
        const oldCardIds = new Set(oldDeck.cards.map(c => c.id));
        const newCards = updatedDeck.cards.filter(c => !oldCardIds.has(c.id));

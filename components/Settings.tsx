...
const Settings: React.FC<SettingsProps> = ({ user, onSave, onLogout, isGuest = false, onLogin }) => {
  ...
  return (
    <div className="p-6 md:p-8 h-full overflow-y-auto max-w-4xl animate-fade-in">
      ...
      <p className="text-gray-500 mb-8">Personalizează-ți experiența de învățare</p>
      ...
      <div className={`bg-[#F8F6F1] p-8 rounded-3xl mb-6 shadow-sm ${isGuest ? 'opacity-60 pointer-events-none' : ''}`}>
        ...
        <div className="space-y-4 max-w-lg">
          <div>
            <label className="block text-gray-500 text-sm mb-1 font-medium">Nume complet</label>
            <input
              name="name"
              type="text"
              value={formData.name}
          ...
          <div>
            <label className="block text-gray-500 text-sm mb-1 font-medium">Email</label>
            <input
              name="email"
              type="email"
      ...
      {onLogout && !isGuest && (
        <div className="bg-red-50 p-6 rounded-3xl border border-red-100 mb-10">
          ...
          <p className="text-red-600 text-sm mb-4">
            Te vei deconecta din cont și vei fi redirecționat către pagina de autentificare.
          </p>
          <button
            onClick={onLogout}

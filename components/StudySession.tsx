...
const StudySession: React.FC<StudySessionProps> = ({ 
  ...
  const hasSavedSession = !!deck.sessionData && Object.keys(deck.sessionData.answers).length < deck.cards.length;
  ...
  const [activeCards, setActiveCards] = useState<Card[]>(() => {
    if (hasSavedSession && deck.sessionData?.shuffledOrder) {
       ...
       const ordered = deck.sessionData.shuffledOrder.map(id => orderMap.get(id)).filter(Boolean) as Card[];
       return ordered.length > 0 ? ordered : [...deck.cards];
    }
    return [...deck.cards];
  });
  
  const [currentIndex, setCurrentIndex] = useState(() => hasSavedSession ? deck.sessionData!.currentIndex : 0);
  const [answers, setAnswers] = useState<Record<string, AnswerStatus>>(() => hasSavedSession ? deck.sessionData!.answers : {});
  const [streak, setStreak] = useState(() => hasSavedSession ? deck.sessionData!.streak : 0);
  const [sessionXP, setSessionXP] = useState(() => hasSavedSession ? deck.sessionData!.sessionXP : 0);
  const [awardedCards, setAwardedCards] = useState<Set<string>>(() => hasSavedSession ? new Set(deck.sessionData!.awardedCards) : new Set());
  ...
  const [floatingXP, setFloatingXP] = useState<{show: boolean, id: number}>({ show: false, id: 0 });
  ...
  }, [answers, streak, sessionXP, awardedCards, currentIndex, activeCards, isFinished, deck.id, onSaveProgress]);
  ...
  const handleRestartSession = () => {
    if (confirm("Ești sigur că vrei să începi de la zero? Progresul sesiunii curente se va pierde.")) {
        // Reset everything
      ...
      <span className="italic">
        {parts.map((part, i) => 
          part.toLowerCase() === word.toLowerCase() 
            ? <strong key={i} className="text-gray-900 not-italic border-b-2 border-yellow-300">{part}</strong> 
            : part
        )}
      </span>
    );
  ...
  const handleReviewMistakes = () => {
    const incorrectIds = Object.keys(answers).filter(id => answers[id] === 'incorrect' || answers[id] === 'skipped');
  ...
  const handleAnswer = useCallback((status: 'correct' | 'incorrect') => {
  ...
  const checkInputAnswer = (e?: React.FormEvent) => {
      ...
      const isMatch = correctAnswers.some(ans => ans === userAnswer || (ans.length > 3 && userAnswer.includes(ans)));
  ...
  }, [isFinished, isFlipped, currentCard, goToPrevious, handleSkip, handleAnswer, handleFlip, editingCard, userInputValue, currentAnswer, goToNext]);


  const calculateScore = () => Object.values(answers).filter(a => a === 'correct').length;
  const incorrectTotal = Object.values(answers).filter(a => a === 'incorrect' || a === 'skipped').length;
  ...
  if (activeCards.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center">
         ...
         <button onClick={onBack} className="bg-gray-900 text-white px-6 py-2 rounded-lg font-medium">Înapoi</button>
      </div>
    );
  }
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 bg-[#F8F6F1]">
        <div className="bg-white p-8 rounded-3xl shadow-xl text-center max-w-md w-full animate-pop-in">
          <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${isPerfect ? 'bg-green-100' : 'bg-orange-100'}`}>
            {isPerfect ? <CheckCircle className="text-green-600 w-10 h-10" /> : <RefreshCcw className="text-orange-600 w-10 h-10" />}
          </div>
  ...
  return (
    <div className="h-full flex flex-col p-4 md:p-6 max-w-4xl mx-auto relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-6 z-10 relative">
        <button onClick={onBack} className="p-2 -ml-2 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0">
          <ChevronLeft size={24} />
        </button>
        ...
        {currentCard.type === 'standard' ? (
          <div className="w-full max-w-lg aspect-[4/3] relative group">
            ...
            >
              ...
              <div className="absolute inset-0 backface-hidden bg-white border-2 border-[#E5E7EB] rounded-[2rem] shadow-xl flex flex-col items-center justify-center p-6 md:p-10 text-center z-20 overflow-hidden">
                 ...
                 <div className="absolute top-4 right-4 flex gap-2 z-50 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                    <button 
                 ...
                 <button 
                     onClick={(e) => { e.stopPropagation(); handleUseHint(); }}
                 ...
                 {currentAnswer && (
                   <div className={`absolute top-6 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-bold uppercase shadow-sm whitespace-nowrap z-40
                     ${currentAnswer === 'correct' ? 'bg-green-100 text-green-700' : 
                       currentAnswer === 'incorrect' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}
                   `}>
                     {currentAnswer === 'correct' ? 'Știut' : currentAnswer === 'incorrect' ? 'Neștiut' : 'Sărit'}
                   </div>
                 )}
                     ...
                     <div className="absolute top-20 left-0 right-0 px-8 animate-fade-in pointer-events-none z-30">
                         <div className="bg-yellow-50 border border-yellow-100 text-yellow-800 text-sm p-4 rounded-xl shadow-sm text-center">
                             ...
                             <div className="text-yellow-900 font-medium flex flex-col sm:flex-row items-center justify-center gap-1">
                                ...
                                <span className="text-xl font-bold bg-white/50 px-2 rounded">{currentCard.back.substring(0, 1).toUpperCase()}...</span>
                             </div>
                         </div>
                     </div>
                 )}
                 
                 <p className="text-xs text-gray-400 uppercase tracking-widest font-bold mb-4 mt-8">Întrebare</p>
                 <h3 className="text-2xl md:text-3xl font-bold text-gray-800 leading-snug select-none mb-6">{currentCard.front}</h3>
                 
                 {/* Input Field on Front */}
                 {!isFlipped && (
                     <div className="w-full max-w-xs relative mt-auto mb-8 z-50" onClick={e => e.stopPropagation()}>
              ...
              {/* BACK */}
              <div className={`absolute inset-0 backface-hidden rotate-y-180 border-2 rounded-[2rem] shadow-xl flex flex-col items-center justify-center p-6 md:p-10 text-center z-20 group
                 ${inputFeedback === 'error' ? 'bg-red-50 border-red-200' : 'bg-[#F0FDF4] border-green-200'}
              `}>
                 {/* Back Controls */}
                 <div className="absolute top-4 right-4 flex gap-2 z-50 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                    <button onClick={openEditModal} className="p-2 bg-white/50 hover:bg-white hover:text-indigo-600 rounded-full transition-colors shadow-sm cursor-pointer">
                        <Edit2 size={16} />
                    </button>
                 </div>
                 ...
                 <p className="text-xs text-gray-400 uppercase tracking-widest font-bold mb-4">Răspuns Corect</p>
                 <h3 className="text-xl md:text-3xl font-medium text-gray-800 leading-relaxed select-none">{currentCard.back}</h3>
              </div>
            </div>
          </div>
        ) : (
          ...
          <div className="w-full max-w-lg bg-white rounded-[2rem] shadow-xl p-8 border border-gray-100 animate-slide-up relative group">
             ...
             <div className="absolute top-4 right-4 flex gap-2 z-50 group-hover:opacity-100 transition-opacity">
                <button onClick={deleteCurrentCard} className="p-2 bg-gray-100 hover:bg-red-100 hover:text-red-600 rounded-full transition-colors cursor-pointer">
                    <Trash2 size={16} />
                </button>
             </div>
             
            <h3 className="text-2xl font-bold text-gray-800 text-center mb-8">{currentCard.front}</h3>
      ...
      <div className="mt-8 flex items-center justify-between gap-4 max-w-lg mx-auto w-full z-10 relative">
        ...
        >
          {currentIndex === activeCards.length - 1 ? <CheckCircle className="text-indigo-600" size={20} /> : <SkipForward size={20} />}
        </button>
      </div>
      
      {/* Keyboard Hint */}
      <div className="hidden md:block text-center mt-4 text-xs text-gray-400 font-medium">
        [Space/Enter] Verifică/Întoarce &nbsp;•&nbsp; [←] Anterior / Greșit &nbsp;•&nbsp; [→] {currentAnswer === 'incorrect' ? 'Continuă' : 'Următor / Corect'}
      </div>

      {/* EDIT MODAL - Same as before */}
      {editingCard && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in" onClick={() => setEditingCard(null)}>
           <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl animate-scale-up" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-4">
                 <h3 className="font-bold text-lg">Editează Card</h3>
                 <button onClick={() => setEditingCard(null)} className="text-gray-400 hover:text-gray-900"><X size={20} /></button>
              </div>
              <form onSubmit={saveEdit} className="space-y-4">
                 <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Față (Întrebare)</label>
                    <textarea 
                 ...
                 <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Spate (Răspuns)</label>
                    <textarea 
                       className="w-full border-2 border-gray-100 rounded-xl p-3 font-medium focus:border-indigo-500 outline-none resize-none"
                       rows={3}
